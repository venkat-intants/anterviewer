...
Matrix:
  happy path            owned session + 2 events → 200, score computed, stored=2
...
def _patch_db(*, session_user_id: str | None, event_rows: list[tuple]) -> Any:
...
async def test_post_integrity_happy_path(client: AsyncClient) -> None:
    """Owned session + events → 200, score computed from the stored events."""
    uid = str(uuid.uuid4())
    sid = str(uuid.uuid4())
