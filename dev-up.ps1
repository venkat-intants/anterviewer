...
.DESCRIPTION
    ...
    The 5 processes:
      1. data_gateway      :8002   auth / users / DPDP consent   (Poetry venv)
      ...
      5. web               :5173   React/Vite frontend
...
.NOTES
    interview_core / feedback_billing use the in-project .venv python directly.
    Do NOT `poetry install` interview_core (its lock is out of sync with the
    pip-managed LiveKit stack). data_gateway uses Poetry's own venv.
#>
...
# 1. data_gateway — auth (:8002) — Poetry venv
...
# 6. web — frontend (:5173)
Start-Svc 'web :5173' $web 'npm run dev'
...
Write-Host '  web (frontend)   http://localhost:5173'
Write-Host ''
Write-Host 'Once every window shows it has started, open http://localhost:5173' -ForegroundColor Cyan
