    ...
    <div className="max-w-3xl space-y-6">
      ...
      <div className="flex items-center justify-between gap-3">
        <div className="min-w-0">
          ...
          <h1 className="truncate text-2xl font-bold text-foreground">{exam.title}</h1>
        </div>
      ...
      <Card className="shadow-sm">
        <CardHeader className="pb-3">
          <CardTitle className="text-base">Questions ({exam.questions.length})</CardTitle>
        ...
        <CardContent className="space-y-3">
                ...
                {q.options.map((opt, oi) => (
                  <li
                    ...
                    className={cn(
                      ...
                      oi === q.correct_index ? 'font-medium text-emerald-700' : 'text-muted-foreground',
                    )}
                  >
                    {oi === q.correct_index ? (
          ...
          {!locked && (
            <form onSubmit={submitQuestion} className="space-y-2 rounded-lg border border-dashed border-input p-3">
              <Input
      ...
      <Card className="shadow-sm">
        <CardHeader className="pb-3">
          <CardTitle className="text-base">Assign &amp; share links</CardTitle>
          <CardDescription>
            {isPublished
        ...
        <CardContent className="space-y-3">
                ...
                <div className="space-y-1.5 rounded-md border border-emerald-200 bg-emerald-50 p-2.5">
                  ...
                  {minted.map((m) => (
                    <div key={m.assignment_id} className="flex items-center gap-2 text-xs">
                      <span className="w-32 shrink-0 truncate font-medium">{m.applicant_name}</span>
                      <input readOnly value={m.magic_link} className="min-w-0 flex-1 rounded border border-border bg-background px-2 py-1" />
                      <button
                        type="button"
          ...
          {(assignments ?? []).length > 0 && (
            <div className="space-y-1">
              ...
              {(assignments ?? []).map((a) => (
                <div key={a.assignment_id} className="flex items-center gap-2 text-sm">
